...
def list_email_templates(
    ...
    if search:
        query = query.or_(f"key.ilike.%{search}%,name.ilike.%{search}%,subject_template.ilike.%{search}%")
    query = query.order("name")
    if limit > 0:
        query = query.range(offset, offset + limit - 1)
    response = query.execute()
    rows = response.data or []
    return EmailTemplateListResponse(items=[EmailTemplateOut(**r) for r in rows], count=response.count or 0)
